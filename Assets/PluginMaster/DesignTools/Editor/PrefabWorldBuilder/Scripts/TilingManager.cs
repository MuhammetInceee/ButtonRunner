...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
        ...
        {
            Paint(TilingManager.settings, PAINT_CMD, true, false, TilingData.nextHexId);
            TilingData.SetNextId();
        }
        ...
        private static void TilingStrokePreview(Camera camera)
        {
            ...
            _paintStroke.Clear();
            var settings = TilingManager.settings;
            var boundsDictionary = new Dictionary<int, Bounds>();
            for (int i = 0; i < brushstroke.Length; ++i)
            {
                ...
                if (boundsDictionary.ContainsKey(prefabId)) bounds = boundsDictionary[prefabId];
                else
                {
                    bounds = BoundsUtils.GetBoundsRecursive(prefab.transform);
                    boundsDictionary.Add(prefabId, bounds);
                }
