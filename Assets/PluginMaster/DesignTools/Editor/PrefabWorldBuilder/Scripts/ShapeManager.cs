    ...
    public static partial class PWBIO
    {
        ...
        private static void ShapeToolEditMode(SceneView sceneView)
        {
            ...
            if (_editingPersistentShape && _selectedPersistentShapeData != null)
            {
                ...
                ShapeStrokePreview(sceneView.camera);
            }
            if (Event.current.type == EventType.KeyDown && Event.current.keyCode == KeyCode.Return)
            {
                void DeleteObjectsOutTheShape()
                {
        ...
        private static void PreviewPersistenShape(ShapeData shapeData)
        {
            ...
            _objectsOutOfTheLine = objList.ToList();
        ...
        private static void ApplySelectedPersistentShape(bool deselectPoint)
        {
            ...
            {
                _initialPersistentShapeData = null;
                _selectedPersistentShapeData = null;
                return;
            }

            selectedShape.UpdatePoses();
            if (_paintStroke.Count > 0)
            {
                var objDic = Paint(ShapeManager.settings, PAINT_CMD, true);
                foreach (var item in objDic)
                {
                    var shape = ShapeManager.instance.GetItem(item.Key);
                    if (shape == null) continue;
                    shape.AddObjects(item.Value.ToArray());
                }
            }
            if (deselectPoint)
            {
            ...
            _initialPersistentShapeData = selectedShape.Clone();
            var persistentShapes = ShapeManager.instance.GetPersistentItems();
            if (!deselectPoint) return;
            foreach (var shape in persistentShapes)
            {
                shape.selectedPointIdx = -1;
