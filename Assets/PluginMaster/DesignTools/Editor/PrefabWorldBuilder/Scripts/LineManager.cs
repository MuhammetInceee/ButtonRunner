...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
        ...
        private static List<GameObject> _objectsOutOfTheLine = new List<GameObject>();
        ...
        private static void PreviewPersistenLine(LineData lineData)
        {
            ...
            _objectsOutOfTheLine.AddRange(lineData.objects.ToList());
            float pathLength = 0;
            ...
            _objectsOutOfTheLine = _objectsOutOfTheLine.Where(i => i != null).ToList();
            foreach (var obj in _objectsOutOfTheLine)
                obj.SetActive(false);
        }
        ...
        private static void ApplySelectedPersistentLine(bool deselectPoint)
        {
            ...
            {
                var objDic = Paint(LineManager.settings, PAINT_CMD, true, true);
                foreach(var item in objDic)
                {
                    var line = LineManager.instance.GetItem(item.Key);
                    if (line == null) continue;
                    line.AddObjects(item.Value.ToArray());
                }
            }
            if (deselectPoint)
            {
        ...
        private static void LineStrokePreview(Camera camera, LineData lineData, bool persistent = false)
        {
            ...
            for (int i = 0; i < brushstroke.Length; ++i)
            {
                ...
                Transform parentTransform = GetParent(settings, prefab.name, false, LineData.nextHexId);
